...
//--- instance.pas
program p(output);
type t(n: integer) = record c0: char;
       a: packed array[1..3] of integer;
       s: string(n) end;
var v: t(4); i: integer;
begin
  v.c0 := 'A'; v.s := 'hi';
  for i := 1 to 3 do v.a[i] := i * 9;
  write(v.c0, ' ');
  for i := 1 to 3 do write(v.a[i]:1, ' ');
  writeln(v.s);
end.
...
begin
  new(q, 4);
  q^.c0 := 'A'; q^.s := 'hi';
  for i := 1 to 3 do q^.a[i] := i * 9;
  write(q^.c0, ' ');
  for i := 1 to 3 do write(q^.a[i]:1, ' ');
  writeln(q^.s);
end.
