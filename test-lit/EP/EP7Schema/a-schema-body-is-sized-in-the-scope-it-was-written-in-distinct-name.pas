...
Control case: alloc's own local is spelled differently (kk, not k), so
there is no shadowing at all.  Paired with ...-shadowed-name.pas, which
differs only in that local's spelling and must produce the identical
output -- the object's layout must not depend on the name of an
unrelated local.
*)
