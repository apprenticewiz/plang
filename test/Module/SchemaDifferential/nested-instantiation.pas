...
type inner(m: integer) = array[1..m] of integer;
     t(n: integer) = record a: array[1..n] of integer; x: inner(n); k: integer end;
var v: t(4); i: integer;
begin
  for i := 1 to 4 do begin v.a[i] := i; v.x[i] := i * 100 end;
  v.k := 99;
  writeln(v.x[4]:1, ' ', v.k:1);
end.
...
begin
  for i := 1 to 4 do begin a.a[i] := i; a.x[i] := i * 100 end;
  a.k := 99;
  rd(a)
end.
