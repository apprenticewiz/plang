...
(*
CHECK:7 14 21 28 35 42 abcdef 4242
*)

//--- instance.pas
program p(output);
type t(n: integer) = record a: array[1..n] of integer; s: string(n); k: integer end;
var v: t(6); i: integer;
begin
  for i := 1 to 6 do v.a[i] := i * 7;
  v.s := 'abcdef'; v.k := 4242;
  for i := 1 to 6 do write(v.a[i]:1, ' ');
  writeln(v.s, ' ', v.k:1);
end.
...
begin
  new(q, 6);
  for i := 1 to 6 do q^.a[i] := i * 7;
  q^.s := 'abcdef'; q^.k := 4242;
  for i := 1 to 6 do write(q^.a[i]:1, ' ');
  writeln(q^.s, ' ', q^.k:1);
end.
