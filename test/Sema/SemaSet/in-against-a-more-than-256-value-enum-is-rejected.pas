...
(* Naming a >256-value enum directly in `set of big` is already rejected by
   checkSetBaseRange (SemaType.cpp).  `x in [...]` reaches the very same
   unchecked-width hazard from the other side: isLooseSet's window is derived
   from x's own ordinal type via Ctx_.getSet(Lt, false) with no call to
   checkSetBaseRange at all, so a >256-value enum used as the left operand of
   `in` silently synthesized a set type whose declared span the 256-bit
   bitmask cannot hold.  Codegen then truncated the high member out of the
   mask instead of ever reporting it: `e in [v256]` read as false for
   e = v256. *)
program p;
type big = (
