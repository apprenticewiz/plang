...
(* checkSetLit's untyped/no-target-hint fallback (used by card(), or a set
   literal with no other inferred type) only derives and width-checks a
   window off its own elements when the element type is plain Integer, via
   literalSetWindow's constant folding -- see the sibling
   literal-set-too-wide_positive-only.pas/_negative-window.pas.  It never
   called checkSetBaseRange for an Enum-typed (or other non-Integer-ordinal)
   element the way a named `set of Base` (SemaType.cpp) and the `x in [...]`
   adoption path (issue #227) both do, so a set literal built from a
   >256-value enum silently dropped members past bit 255 in the runtime
   bitmask instead of being diagnosed: `card([e0, e299])` for a 300-value
   enum read as 1 instead of 2. *)
program p;
type big = (
