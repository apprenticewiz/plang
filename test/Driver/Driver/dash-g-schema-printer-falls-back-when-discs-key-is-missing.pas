...
A schema entry missing "discs" entirely has no way to read the
discriminants a run-time-varying field's bound depends on -- there is no
single field to blame, so the top-level lookup function now returns None
for it, and gdb falls back to its own default DWARF-only printing instead
of a broken pretty-printer or a raw traceback.
*)
...
RUN: python3 -c "import json; p='%s.plang-schemas.json'; d=json.load(open(p)); del d['schemas']['Rec'][0]['discs']; json.dump(d, open(p, 'w'))"
